...
4. 拡張アイコンをクリックすると記録一覧（既定は今開いている配信のみ。チェックを外すと全件）。「補正」で視聴遅延分の秒数を調整（例: 配信が 10 秒遅れて見えているなら `-10`）。メモ欄をクリックすると一言メモを付けられる。各行のリンクは `watch?v=...&t=...s` で VOD 位置を開く。「テキストをコピー」で動画ごとの `h:mm:ss` 一覧（コメント・概要欄用）、「JSON」で全件バックアップ
