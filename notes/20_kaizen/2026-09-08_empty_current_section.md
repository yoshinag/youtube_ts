...
| GDR ID | 決定の要約 | status |
|---|---|---|
| GDR-UI-005 | popup の一覧は記録がある配信だけを出す。現在の配信に記録が無くても空セクションは作らない（GDR-UI-004 の「空セクションを先頭に出す」を撤回） | Implemented |
...
| # | タスク | 根拠 GDR | 依存 | ステータス |
|---|---|---|---|---|
| 1.1 | `groupRecordsByStream` の空グループ挿入を削除、テスト反転 | UI-005 | — | 完了 |
| 1.2 | `view.ts` / `index.html` の空行・文言整理、typecheck / test / build | UI-005 | 1.1 | 完了 |
...
- 空グループ挿入を消したら `current` 引数の `streamStartAt` も不要になり、シグネチャが小さくなった。UI-004 のときに「記録直後に同じ場所へ行が増える」ために足した仕様は、実際には全削除の見え方を悪くしていただけだった
- 純関数のテストを反転させるだけで仕様変更が完了し、popup 側は削除のみ

### 7.2. 次回への申し送り

- 2.1 実機検証（全削除後にセクションが残らない、記録すると先頭に現れる）は未実施
- 「今どの配信を開いているか」を出したくなったら代替案 B（ツールバーに現在の配信名）
