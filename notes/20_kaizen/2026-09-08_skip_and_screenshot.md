...
| GDR ID | 決定の要約 | status |
|---|---|---|
| GDR-DOM-002 | スキップとフレーム取得は `<video>` 要素を直接操作する（`currentTime` 加減算 / `canvas.drawImage`）。記録・スクショの経過秒は「実時刻 − 配信開始」から `seekable.end − currentTime`（ライブ端からの遅れ）を引いて再生位置に追従させる | Proposed |
| GDR-EXT-002 | スクリーンショットは `downloads` 権限で `Downloads/ss/` に保存し、実際の保存先は OS のシンボリックリンクで `/Users/yn_mini_0/_works/ss` に向ける。「フォルダを開く」は `downloads.show` | Proposed |
...
**GDR-EXT-002: スクリーンショットは `downloads` 権限で `Downloads/ss/` に保存し、絶対パスは OS 側のシンボリックリンクで解決する**
...
- **決定:**
  ...
  - `/Users/yn_mini_0/_works/ss` に置きたい要望は、利用者が一度だけ `ln -s /Users/yn_mini_0/_works/ss ~/Downloads/ss` を実行して解決する（README に手順を書く）。拡張はサブフォルダ名 `ss` だけを知っている
...
- **理由:**
  ...
  - **代替案 D: Chrome の既定ダウンロード先を `/Users/yn_mini_0/_works` に変更** → すべてのダウンロードに影響する。却下（シンボリックリンクの方が影響が局所的）
- **影響:**
...
- `downloads` 権限を追加した理由
- 保存先を `/Users/yn_mini_0/_works/ss` にする手順: `ln -s /Users/yn_mini_0/_works/ss ~/Downloads/ss`
